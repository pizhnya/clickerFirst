...
function spawnParticle(big = false) {
  const p = new THREE.Mesh(new THREE.SphereGeometry(big ? 0.18 : 0.1, 8, 8), new THREE.MeshStandardMaterial({ color: big ? "#ffd166" : "#ffffff", emissive: big ? "#c69200" : "#1f2937" }));
  const origin = enemyMesh?.position || new THREE.Vector3(0, 1, -2);
